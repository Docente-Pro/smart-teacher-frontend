...
import {
  ...
  getHistorialPagosUnidad,
  confirmarPagoUnidad,
  confirmarPagoSuscriptor,
  rechazarPagoUnidad,
} from "@/services/admin.service";
import type { IPagoUnidadAdmin, EstadoPago } from "@/interfaces/IAdmin";
...
export default function AdminPagosUnidad() {
  const [tab, setTab] = useState<Tab>("pendientes");
  const [pagos, setPagos] = useState<IPagoUnidadAdmin[]>([]);
  ...
  async function cargarPagos() {
    ...
    try {
      const res =
        tab === "pendientes"
          ? await getPagosUnidadPendientes()
          : await getHistorialPagosUnidad(estadoFilter || undefined);
      setPagos(res.data || []);
    } catch (err) {
      toast.error("Error al cargar pagos de unidad");
    } finally {
      setIsLoading(false);
    }
  }

  async function handleConfirmar(pago: IPagoUnidadAdmin) {
    const esSuscriptor = pago.tipoPago === "SUSCRIPTOR";
    const label = esSuscriptor ? "pago de suscriptor" : "pago de unidad";
    if (!window.confirm(`¿Confirmar este ${label}?`)) return;

    setActionLoading(pago.id);
    try {
      if (esSuscriptor) {
        await confirmarPagoSuscriptor(pago.id);
      } else {
        await confirmarPagoUnidad(pago.id);
      }
  ...
  async function handleRechazar(pagoId: string) {
    ...
    if (!window.confirm("¿Rechazar este pago de unidad?")) return;
    setActionLoading(pagoId);
    try {
      await rechazarPagoUnidad(pagoId, motivo || undefined);
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-gray-400 mt-1 text-sm">
            Gestiona los pagos de unidades y suscriptores
          </p>
        </div>
        <Button
          variant="outline"
                ...
                {pagos.map((pago) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      {pago.unidad ? (
                        <div>
                          <p className="text-white text-xs truncate max-w-[140px]">
                            {pago.unidad.titulo}
                          </p>
...
function TipoBadge({ tipo }: { tipo: string }) {
  const config: Record<string, { bg: string; text: string; label: string }> = {
    PRE_PAGO: { bg: "bg-green-500/10", text: "text-green-400", label: "Pre-Pago" },
    PROPIETARIO: { bg: "bg-blue-500/10", text: "text-blue-400", label: "Propietario" },
    SUSCRIPTOR: { bg: "bg-purple-500/10", text: "text-purple-400", label: "Suscriptor" },
  };
  const c = config[tipo] || config.PROPIETARIO;
  return (
