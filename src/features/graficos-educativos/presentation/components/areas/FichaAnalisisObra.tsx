    ...
    <div className="space-y-4">
      ...
      <div className="bg-amber-50 rounded-xl p-4 flex items-start gap-3">
        ...
        <div className="text-sm">
          <p className="font-bold text-gray-800">{obra.nombre}</p>
          <p className="text-gray-600">
            {obra.autor} — {obra.origen}
          </p>
          <span className="inline-block mt-1 text-xs bg-amber-200 text-amber-800 px-2 py-0.5 rounded-full capitalize">
            {obra.tipo}
          </span>
        </div>
      </div>
        ...
        {dimensiones.map((dim, i) => (
          ...
          >
            ...
            <div>
              ...
              <p className="text-sm text-gray-700">{dim.observacion}</p>
            </div>
          </div>
        ))}
      </div>
      ...
      {miOpinion && (
        <div className="bg-purple-50 border border-purple-200 rounded-lg p-4">
          ...
          <p className="text-sm text-gray-700 italic">{miOpinion}</p>
          <div className="mt-3 w-full h-10 border-b-2 border-dashed border-purple-300" />
        </div>
      )}
    </div>
  );
